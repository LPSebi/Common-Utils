...
public class PlayerHandlers
{
    ...
    public void OnChangingRole(ChangingRoleEventArgs ev)
    {
        ...
        if (config.StartingInventories.ContainsKey(ev.NewRole) && !ev.ShouldPreserveInventory)
        {
            ...
            if (config.StartingInventories[ev.NewRole].Ammo == null ||
                config.StartingInventories[ev.NewRole].Ammo.Count <= 0)
                return;

            if (config.StartingInventories[ev.NewRole].Ammo.Any(s =>
                    string.IsNullOrEmpty(s.Group) || s.Group == "none" ||
                    (ServerStatic.PermissionsHandler._groups.TryGetValue(s.Group, out UserGroup userGroup) &&
                     userGroup == ev.Player.Group)))
            {
                ...
                foreach ((ItemType type, ushort amount, string group) in config.StartingInventories[ev.NewRole].Ammo)
                {
                    if (string.IsNullOrEmpty(group) || group == "none" ||
                        (ServerStatic.PermissionsHandler._groups.TryGetValue(group, out UserGroup userGroup) &&
                         userGroup == ev.Player.Group))
                    {
    ...
    public void OnSpawned(SpawnedEventArgs ev)
    {
        ...
        if (ev.Player.Role is FpcRole && config.PlayerHealthInfo)
        {
            ev.Player.CustomInfo =
                $"({ev.Player.Health}/{ev.Player.MaxHealth}) {(!string.IsNullOrEmpty(ev.Player.CustomInfo) ? ev.Player.CustomInfo.Substring(ev.Player.CustomInfo.LastIndexOf(')') + 1) : string.Empty)}";
        }

        if (config.AfkIgnoredRoles is not null && config.AfkIgnoredRoles.Contains(newRole) &&
            Plugin.AfkDict.TryGetValue(ev.Player, out Tuple<int, Vector3> value))
        {
    ...
    {
        if (ev.Attacker is not null && config.HealthOnKill is not null &&
            config.HealthOnKill.ContainsKey(ev.Attacker.Role))
        {
    ...
    {
        if (config.RoleDamageDealtMultipliers is not null && ev.Attacker is not null &&
            config.RoleDamageDealtMultipliers.TryGetValue(ev.Attacker.Role, out var damageMultiplier))
            ev.Amount *= damageMultiplier;

        if (config.RoleDamageReceivedMultipliers is not null &&
            config.RoleDamageReceivedMultipliers.TryGetValue(ev.Player.Role, out damageMultiplier))
            ev.Amount *= damageMultiplier;

        if (config.DamageMultipliers is not null &&
            config.DamageMultipliers.TryGetValue(ev.DamageHandler.Type, out damageMultiplier))
            ev.Amount *= damageMultiplier;
    ...
    {
        if (ev.Player.IsCuffed && config.DisarmedEscapeSwitchRole is not null &&
            config.DisarmedEscapeSwitchRole.TryGetValue(ev.Player.Role, out RoleTypeId newRole))
        {
            ev.NewRole = newRole;
    ...
    public List<ItemType> GetStartingInventory(RoleTypeId role, Player player = null)
    {
        ...
        Log.Debug($"{nameof(this.GetStartingInventory)} Iterating through slots...");
        ...
        for (int i = 0; i < config.StartingInventories[role].UsedSlots; i++)
        {
            ...
            Log.Debug(
                $"{nameof(GetStartingInventory)} Finished checking groups, found {itemChances.Count} valid itemChances.");
            ...
            {
                Log.Debug(
                    $"{nameof(GetStartingInventory)} Trying to assign to slot {i + 1} for {role}; item: {item}; {rolledChance} <= {chance} ({rolledChance <= chance}).");
                ...
                if (rolledChance <= chance)
                {
                    ...
                    if (CustomItem.TryGet(item, out CustomItem customItem))
                    {
                        if (player != null)
                            ...
                            Log.Warn(
                                $"{nameof(GetStartingInventory)} Tried to give {customItem!.Name} to a null player.");

                        break;
                    }

                    Log.Warn(
                        $"{nameof(GetStartingInventory)} Skipping {item} as it is not a valid ItemType or CustomItem!");
                }
